...
// 				}
    			...
    			return $this->redirect($this->generateUrl('almo_wallet_wallet_wallet_1', array('act' => $operations->getType())));
    		}
    		
    	}
    	
    	return array('form' => $form->createView(), 'act' => $act);
    ...
    public function statusAction($accountId)
    {
    	...
    	if ($accountId)
    	{
    		$payments = $em->getRepository('AlmoWalletBundle:Payments')->getUserAccountPayments($accountId, $this->get('security.token_storage')->getToken()->getUser());
    	}
    	
    	return array('accounts' => $accounts, 'payments' => $payments);
